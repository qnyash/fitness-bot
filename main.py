...
def callback_query(call):
    ...
    # ---------- ТРЕНИРОВКИ ----------
        ...
        program = active_workouts[user_id]["program"]
        sets_count = safe_int(program[ex_idx].get("sets", 1), 1)

        current = completed_sets.get(ex_idx, [])

        if len(current) > 0:
            completed_sets[ex_idx] = []
        else:
            completed_sets[ex_idx] = list(range(sets_count))
        ...
        program = workout["program"]
        day = workout["day"]
        is_no_power_mode = (day == "СилыНет")

        all_done = True

        for i in range(len(program)):
            if is_no_power_mode:
                if len(completed_sets.get(i, [])) == 0:
                    all_done = False
                    break
            else:
